...
const LIST = gql`
  query WorkOrderStatusesAdmin($includeInactive: Boolean = true) {
    workOrderStatusesAdmin(includeInactive: $includeInactive) {
      id
      name
      isDefault
      active
...
const CREATE = gql`
  mutation CreateWorkOrderStatus($input: WorkOrderStatusCreateInput!) {
    createWorkOrderStatus(input: $input) {
      id
      name
      isDefault
      active
...
const UPDATE = gql`
  mutation UpdateWorkOrderStatus($id: ID!, $input: WorkOrderStatusUpdateInput!) {
...
const DELETE = gql`
  mutation DeleteWorkOrderStatus($id: ID!) {
    deleteWorkOrderStatus(id: $id)
  }
`;

const SET_DEFAULT = gql`
  mutation SetDefaultWorkOrderStatus($id: ID!) {
    setDefaultWorkOrderStatus(id: $id) {
      id
      name
      isDefault
      active
...
const REORDER = gql`
  mutation ReorderWorkOrderStatuses($ids: [ID!]!) {
    reorderWorkOrderStatuses(ids: $ids)
  }
`;

export default function WorkOrderStatusesPage({ inDialog = false }: { inDialog?: boolean }) {
  ...
  const { data, loading, error, refetch } = useQuery<{ workOrderStatusesAdmin: Item[] }>(LIST, {
  ...
  const rows = useMemo(() => data?.workOrderStatusesAdmin ?? [], [data?.workOrderStatusesAdmin]);
  ...
  const onRowReorder = useCallback(
    async (e: any) => {
      ...
      // With pagination, e.value contains the reordered current page slice
  ...
  async function handleCreate() {
    ...
    // Client-side duplicate guard for immediate friendly message
    ...
    try {
      ...
      await refetch();
      toast.current?.show({ severity: 'success', summary: 'Created', detail: 'Status added.' });
    } catch (e: any) {
  ...
  async function saveEdit() {
    ...
    try {
      ...
      cancelEdit();
      await refetch();
      toast.current?.show({ severity: 'success', summary: 'Saved', detail: 'Status updated.' });
    } catch (e: any) {
  ...
  async function makeDefault(m: Item) {
    try {
      ...
      await refetch();
      toast.current?.show({ severity: 'success', summary: 'Updated', detail: 'Default status set.' });
    } catch (e: any) {
  ...
  async function handleDelete() {
    ...
    try {
      ...
      await refetch();
      toast.current?.show({ severity: 'success', summary: 'Deleted', detail: 'Status deleted.' });
    } catch (e: any) {
  ...
  try { console.debug('[WorkOrderStatuses] isSuperAdmin =', isSuperAdmin); } catch {}
  return (
    <div className="grid">
      <Toast ref={toast} />
      <div className="col-12">
        <Card>
          {!inDialog ? (
            <div className="mb-4">
              <h2 className="m-0">Work Order Statuses</h2>
            </div>
          ) : null}
          ...
          <DataTable
            ...
            emptyMessage={loading ? 'Loading…' : 'No statuses found.'}
      ...
      >
        <div className="grid">
          <div className="col-12">
            ...
            <InputText value={newItem.name} onChange={(e) => setNewItem((p) => ({ ...p, name: e.target.value }))} placeholder="e.g., Scheduled" className="w-full" />
          </div>
      ...
      >
        ...
        <small className="text-600">This may fail if the status is referenced by existing work orders.</small>
      </Dialog>
    </div>
  );
}