...
import { getMenuModel } from './AppMenu'; // Shared helper ensures per-app menus flow through

const ROOT_LABEL = (import.meta as any).env?.VITE_BREADCRUMB_ROOT || (import.meta as any).env?.VITE_APP_TAGLINE || (import.meta as any).env?.VITE_APP_NAME || 'VidhiOne';
...
const AppBreadcrumb = () => {
    ...
    const breadcrumb = useMemo(() => {
        ...
        if (location.pathname.includes('/fleet/vehicles/') && !location.pathname.endsWith('/list') && !location.pathname.endsWith('/add') && !location.pathname.endsWith('/overview')) {
            ...
            activeBreadcrumb = {
                labels: [ROOT_LABEL, 'Vehicles', `Vehicle #${vehicleId}`],
            ...
            activeBreadcrumb = {
                labels: [ROOT_LABEL, 'Drivers', `Driver ${driverId}`],
                to: location.pathname
            };
        } else {
