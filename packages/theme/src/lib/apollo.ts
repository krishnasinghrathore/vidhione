...
export const isOfflineGraphQL =
    ((import.meta as any).env?.VITE_OFFLINE === '1') ||
    ((import.meta as any).env?.VITE_DISABLE_GRAPHQL === '1');

const offlineLink = new ApolloLink(() => {
    return new Observable((observer) => {
        // Immediately resolve with empty payload to keep UI responsive in UI-only mode
...
export const apolloClient = new ApolloClient({
    link: isOfflineGraphQL ? offlineLink : from([errorLink, retryLink, httpLink]),
    cache: makeCache()
});
