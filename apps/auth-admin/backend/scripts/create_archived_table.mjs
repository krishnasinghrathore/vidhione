...
async function ensureArchivedTableOnly() {
  ...
  try {
    ...
    // Ensure enum exists (safe if exists)
    await client.query(`
DO $$
BEGIN
  IF NOT EXISTS (
...
END $$;
    ...
    // Create only archived_documents table if missing
    await client.query(`
CREATE TABLE IF NOT EXISTS "logiq"."archived_documents" (
  ...
  "archived_by" uuid NULL
    ...
    // FK to document_types (idempotent)
    await client.query(`
DO $$
BEGIN
  IF NOT EXISTS (
...
END $$;
    ...
    // Verify
    const tbl = await client.query(`SELECT table_schema, table_name FROM information_schema.tables WHERE table_schema='logiq' AND table_name='archived_documents'`);
    console.log('[verify table]', tbl.rows);
    ...
    console.log('[done] archived_documents created/verified.');
  } catch (e) {
