...
import type { AppMailSidebarItem, Demo } from '@/types/demo';
...
function AppMailSidebar() {
    ...
    const navigateTo = (item: AppMailSidebarItem) => {
        if (item.to) {
            navigate(item.to);
        }
    };
    ...
    return (
        <React.Fragment>
            <div>
                <Button label="Compose New" className="mb-5 w-full" outlined onClick={(e) => navigate('/apps/mail/compose')}></Button>
                            ...
                            return (
                                <li
                                    ...
                                    className={classNames('p-ripple cursor-pointer select-none p-3 transition-duration-150 border-round flex align-items-center justify-content-center md:justify-content-start md:flex-1 flex-auto', {
                                        'bg-primary': location.pathname === item.to,
                                        'hover:surface-hover': location.pathname !== item.to
                                    })}
                                    onClick={() => navigateTo(item)}
                                >
                                    <i className={classNames('md:mr-3 text-600 transition-duration-150 text-lg', item.icon || '', { 'text-primary-50': location.pathname === item.to })}></i>
                                    <span className={classNames('text-900 font-medium hidden md:inline', { 'text-primary-50': location.pathname === item.to })}>{item.label}</span>
                                    {item.badge ? (
