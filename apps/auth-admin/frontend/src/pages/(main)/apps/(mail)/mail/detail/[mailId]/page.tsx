...
interface MailContextProps {
    ...
    onDeleteMultiple: (mails: Demo.Mail[]) => void;
    onArchiveMultiple: (mails: Demo.Mail[]) => void;
    onSpamMultiple: (mails: Demo.Mail[]) => void;
}
...
const AppMailDetail = () => {
    ...
    const { mails, onSend } = useContext<MailContextProps>(MailContext);

    const sendMail = () => {
        if (newMail.message) {
            setNewMail((prevState: Demo.Mail) => ({
                ...prevState,
                from: mail!.from,
    ...
    useEffect(() => {
        const _mail = mails.filter((m: Demo.Mail) => m.id === parseInt(mailId as string))[0];
        setMail(_mail);
    }, [mails, mailId]);
